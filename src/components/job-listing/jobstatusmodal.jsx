...
export default function JobStatusModal({ isOpen, job, onClose, onConfirm }) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40 font-inter">
      <div className="bg-white rounded-xl p-6 w-96 text-center shadow-lg relative">
        ...
        <h2 className="text-xl font-semibold text-indigo-400 mb-4">{job.status === "Open" ? "Hold Product" : "Show Product"}</h2>
        <p className="text-gray-600 mb-6">
          {job.status === "Open" ? (
            <>
              Are you sure you want to close the <b>{job.job}</b> position?
            </>
          ) : (
            <>
              Do you want to publish the <b>{job.job}</b> position?
            </>
          )}
        </p>
